document.addEventListener("DOMContentLoaded", function(event) {
    // Launch Modal from Article WP
    let arrayURL = document.querySelectorAll('button[data-href]');
    for (let i = 0; i < arrayURL.length; i++) {
        let href = document.getElementsByTagName('button')[i].attributes['data-href'];
        let url = href.value;

        // Create container Modal
        let articleContent = document.getElementById("about");
        let modalDiv = document.createElement('div');
        modalDiv.id = 'modal_' + url;
        articleContent.insertAdjacentHTML('afterend', '<div id="modal_' + url + '"></div>');

        // Load URL in new container
        jQuery('#modal_' + url).load(url);
    }
/*--------------------------------------------------------------*/
    setTimeout(function() {
        if(window.scrollY === 0) {
